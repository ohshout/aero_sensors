...
/* --TC-- just need transfer func for now,
 * may need RC_PinSet() later */
#if 0
...
uint8_t PIOS_SPI_TransferByte(struct pios_spi_dev * spi_dev, uint8_t b)
{
	uint8_t ret = 0;
...
int32_t PIOS_SPI_TransferBlock(struct pios_spi_dev * spi_dev, const uint8_t *send_buffer, uint8_t *receive_buffer, uint16_t len)
{
	//struct pios_spi_dev *spi_dev = (struct pios_spi_dev *)spi_id;
  ...
  struct spi_ioc_transfer xfer = {
	...
	//int status = ioctl(spi_dev->fd[slave_id], SPI_IOC_MESSAGE(1), &xfer);
	int status = ioctl(spi_dev->spi_fd, SPI_IOC_MESSAGE(1), &xfer);
